...
public class TestFlow {
	...
	public static long individualFlowStartTime;
	public static long individualScenarioStartTime;
	...
	public static void execute_TestCase() throws Exception {
		...
		for(int iTestFlow=1; iTestFlow<=iTotaltestFlows; iTestFlow++) {
			individualFlowStartTime = System.currentTimeMillis();
			//-------------
			ExcelUtils.setExelFile(mainFlowFile);
			//------------
			...
			if(sRunMode.equalsIgnoreCase("Yes")) //here checking Runmode of flow eg. Login
			{
               ...
               // System.out.println("inside particular flow file1:::"+FlowFile);
				...
				////////////////////////THIS IS FOR REMOVING THE PREVIOUS RESULT(PASS/FAIL) FROM SCENARIO SHEET
				//////// YOU CAN REMOVE THIS FOR LOOP IF YOU DONT WANT THIS FUNCTIONALITY WITHOUT DISTURBING OTHER CODE
				for (int i = 1; i <= iTotalTestCases-1; i++) {
					ExcelUtils.setCellData("",i,Constants.Col_Result,sTestFlowID,FlowFile);
				}	
				
                ////////////////////////THIS IS FOR REMOVING THE PREVIOUS RESULT(PASS/FAIL) FROM TEST STEP SHEET 
				//////// YOU CAN REMOVE THIS FOR LOOP IF YOU DONT WANT THIS FUNCTIONALITY WITHOUT DISTURBING OTHER CODE
				Sheet_TestSteps=""+sTestFlowID+"_TestSteps";
				int iTotalRowtestStepsSheet = ExcelUtils.getRowcount(Sheet_TestSteps);
				for (int i = 1; i < iTotalRowtestStepsSheet-1; i++) {
					ExcelUtils.setCellData("", i, Constants.Col_TestStepResult, Sheet_TestSteps,FlowFile);
					ExcelUtils.setCellData("", i, Constants.Col_ErrorMsg, Sheet_TestSteps,FlowFile);
				}


				
				//This loop will execute number of times equal to Total number of test cases/scenario
				for(int iTestcase=1;iTestcase<=iTotalTestCases;iTestcase++)
				{
					individualScenarioStartTime = System.currentTimeMillis();
					bResult = true;
				   ...
				   //System.out.println("current running test case::: "+sTestCaseID);
					...
					if (sRunMode.equalsIgnoreCase("Yes")){ //here checking Runmode of each iteration/scenario eg. Login_01
						...
						ActionKeywords.getResultantScreenshot(Constants.KEYWORD_PASS+"-"+sTestFlowID+"-"+sTestCaseID+"-"+sTransitionRow+"-", automationStartTime);
						Log.endTestCase(sTestCaseID);				
						}		
					}
					Log.info("TEST SCENARIO '"+sTestCaseID+"' COMPLETED IN :"+(System.currentTimeMillis()-individualScenarioStartTime)/1000+" Seconds");
					System.out.println("TEST SCENARIO '"+sTestCaseID+"' COMPLETED IN :"+(System.currentTimeMillis()-individualScenarioStartTime)/1000+" Seconds");
				}				
			}
			Log.info("TEST FLOW '"+sTestFlowID+"' COMPLETED IN :"+(System.currentTimeMillis()-individualFlowStartTime)/1000+" Seconds");
			System.out.println("TEST FLOW '"+sTestFlowID+"' COMPLETED IN :"+(System.currentTimeMillis()-individualFlowStartTime)/1000+" Seconds");			
		}
		Log.info("AUTOMATION COMPLETED IN :"+(System.currentTimeMillis()-StartTime)/1000+" Seconds");
		System.out.println("AUTOMATION COMPLETED IN :"+(System.currentTimeMillis()-StartTime)/1000+" Seconds");
		
		ExcelUtils.setExelFile(mainFlowFile);
		ExcelUtils.setCellData(automationStartTime, 1, Constants.Col_RunDate, "Result", mainFlowFile);
        ExcelUtils.setCellData(" "+(testPassCount+testFailCount), 1, Constants.Col_TotalTestCaseRun, "Result", mainFlowFile);
        ExcelUtils.setCellData(" "+testPassCount, 1, Constants.Col_TotalTestCasePass, "Result", mainFlowFile);
        ExcelUtils.setCellData(" "+testFailCount, 1, Constants.Col_TotalTestCaseFail, "Result", mainFlowFile);
        ExcelUtils.setCellData((System.currentTimeMillis()-StartTime)/1000+" Seconds", 1, Constants.Col_TotalRunTime, "Result", mainFlowFile);
		PieChartResult demo = new PieChartResult("Resultant",testPassCount,testFailCount,mainFlowFile,automationStartTime);
	}
		...
		for(int i=0;i<method.length;i++)
		{
			...
			if(method[i].getName().equals(sActionKeyword))
			{
				...
				}else
				{
					...
					ExcelUtils.setCellData(errorMsg, iTestStep, Constants.Col_ErrorMsg, Sheet_TestSteps,FlowFile);
					ActionKeywords.getResultantScreenshot(Constants.KEYWORD_FAIL+"-"+sTestFlowID+"-"+sTestCaseID+"-"+sTransitionRow+"-",automationStartTime);
