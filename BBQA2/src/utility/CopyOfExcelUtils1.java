...
public class CopyOfExcelUtils1 {
	...
	public static int getTestStartRow (String testCaseName, int colNum , String sheetName) throws Exception {
		...
		ExcelWSheet = ExcelWBook.getSheet(sheetName);
		try {
			int sheetTotalRowCount = CopyOfExcelUtils1.getRowcount(sheetName);
			for(i=0; i< sheetTotalRowCount; i++)
			{
				if(CopyOfExcelUtils1.getCellStringData(i, colNum, sheetName).equalsIgnoreCase(testCaseName)) 
					{
						break;
					}
			}
	...
	public static int getTotalTestSteps (String sheetName, String testCaseID, int testCaseStart) throws Exception {
		ExcelWSheet = ExcelWBook.getSheet(sheetName);
		try {
			for (int i = testCaseStart; i <= CopyOfExcelUtils1.getRowcount(sheetName); i++) 
			{
				if(!testCaseID.equals(CopyOfExcelUtils1.getCellStringData(i, Constants.Col_TestCaseID, sheetName)))
				{
					int number = i;
					return number;
				}
