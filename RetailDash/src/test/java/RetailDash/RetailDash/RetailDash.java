...
public class RetailDash {
	...
	public static void main(String[] args) throws Exception {
		...
		int totalSkuRow = ExcelUtils.getRowcount("501-900");
		...
		System.out.println("TOTAL row count need to check from EXCEL SHEET::"+totalSkuRow);
		for(int i=1;i<=totalSkuRow;i++) {  //LOOP FOR TOTAL ROW OF EXCEL SHEET 
			try { 		
			Log.info(":::::::::::::::::Current Product Row from EXCEL Sheet ::"+(i+1));
			String netshoesProduct = ExcelUtils.getCellStringData(i,3,"501-900");  // product brand +color
			String productColor = ExcelUtils.getCellStringData(i, 3, "501-900");  //COLOR FROM EXCEL SHEET
			...
			do {	
				...
				for (int j = 0; j < pdpURLs.length; j++) { //** loop for each searched products				
					try {
						...
						List<WebElement> colorList = searchedProducts.get(j).findElements(By.className("color-list"));  // PRODUCT COLOR LIST ON SEARCH PAGE
					...
					if((j==pdpURLs.length-1)&&productFound == false) {
						ExcelUtils.setCellData("NA", i, 4, "501-900", Path_FileName);  //** Writing Result NA in excel sheet for not matching sku color for all searched product
					}					
					if((j==47)&&productFound == false) {
						ExcelUtils.setCellData("NA. Search result exceed more than 48 product  from 1st page", i, 4, "501-900", Path_FileName);  //** Writing Result NA in excel sheet for not matching sku color for all searched product
						driver.get(searchPageNumberUrl[1]);
					}			
				}  //**END OF EACH SEARCH PRODUCT FOR LOOP
		      } while (!searchPageNumberUrl[0].equalsIgnoreCase(searchPageNumberUrl[1]) && count <3);  //**  END OF DO WHILE LOOP
			}   //**  END OF TRY BLOCK
			catch (Exception e) {
				Log.info("Search page NOT found for Excel Row :"+(i+1)+" Hence skipping this product from EXCEL");
				ExcelUtils.setCellData("NA", i, 4, "501-900", Path_FileName);
				e.printStackTrace();
			}
	...
	public static void writeResultInExcel(int i,int j ,int k) throws Exception {
			...
			//** writing RESULT in excel file
				...
				ExcelUtils.setCellData(productCode, i, 4, "501-900", Path_FileName);
				ExcelUtils.setCellData(requiredUrl, i, 5, "501-900", Path_FileName);
				ExcelUtils.setCellData(colorNameOnCantrino, i, 6, "501-900", Path_FileName);
				ExcelUtils.setCellData(colorCode, i, 7, "501-900", Path_FileName);
	}
