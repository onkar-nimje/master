...
import utility.*;
public class SEOParameterValidation {
	...
	public static final String Path_FileName = "D://workspaces//Selenuim//RetailDash//src//test//java//dataEngine//SEOParameterValidation.xls";                                           
	public static void main(String[] args) throws Exception {
		...
		try {
			for(int i=1;i<=totalRow;i++) {
				...
				System.out.println("Current Row Number: "+i+" URL::"+URL);
				startTime = System.currentTimeMillis();
				System.out.println("Start time::"+startTime);
				driver.get(URL);
				System.out.println("End of page load wait:::"+System.currentTimeMillis());
				System.out.println("TOTAL WAITING TIME IN SECONDS TO LOAD URL FROM ROW NUMBER ("+i+"):: "+((System.currentTimeMillis()-startTime)/1000));
				...
				try {
					...
					//if (title!=null) {
					if (title.length()!=0) {
						ExcelUtils.setCellData(title, i, 2, "Sheet1", Path_FileName);
					} else {
						ExcelUtils.setCellData("NA", i, 2, "Sheet1", Path_FileName);
					}				
				...
				try {
					...
					if (description.length()!=0) {
						ExcelUtils.setCellData(description, i, 3, "Sheet1", Path_FileName);
					} else {
						ExcelUtils.setCellData("NA", i, 3, "Sheet1", Path_FileName);
					}				
				...
				try { 
					...
					//if (h1Tag!=null  ) {	
					if (h1Tag.length()!=0) {
							ExcelUtils.setCellData(h1Tag, i, 5, "Sheet1", Path_FileName);				
					}
					else {
						ExcelUtils.setCellData("NA", i, 5, "Sheet1", Path_FileName);
					}
						...
						try {
							...
							if(canonicalLink.equals("canonical")) {
								...
								if (canonicalTag.length()!=0) {
									ExcelUtils.setCellData(canonicalTag, i, 4, "Sheet1", Path_FileName);
								} else {
									ExcelUtils.setCellData("NA", i, 4, "Sheet1", Path_FileName);
								}							
